...
open class EventView: UIView {
    ...
    private func addCustomView(){
        ...
        let hs=UIStackView(axis: .horizontal)
//        hStack.distribution = .fillProportionally
...
//        textView2.text="Location name"
        textView2.isUserInteractionEnabled = false
        textView2.isScrollEnabled = false
//        tv.backgroundColor = .red
        hs.addArrangedSubview(textView2)
        hStack.addArrangedSubview(hs)
        addSubview(hStack)
    }
    ...
    public func updateWithDescriptor(event: EventDescriptor) {
        ...
        //
        textView2.text=event.text2
        textView2.font=event.font
        textView2.textColor=event.textColor
        //
        let image = UIImage(named: event.imageName)?.withRenderingMode(.alwaysOriginal)
        imageView.image = image;
...
//        hStack.frame=textView.frame
        let newFrame=CGRect(x: bounds.width*0.75, y: bounds.minY, width: bounds.width/3, height: bounds.height)
        hStack.frame=newFrame
